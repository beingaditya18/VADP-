...
class VerificationContractService:
    ...
    async def generate_contract(
        ...
        contract = VerificationContract(
            ...
            evidence_hashes=[e.model_dump() for e in evidence_items],
            evidence_count=len(evidence_items),
            evidence_verified=sum(1 for e in evidence_items if e.verification_status == "verified"),
            # RAG
            ...
            if ev_id:
                ...
                if ev_record:
                    if ev_record.integrity_hash != ev_item.get("integrity_hash"):
                        ...
                        failures.append(f"EVIDENCE TAMPERED: Evidence {ev_id} hash mismatch")
    ...
    async def _record_provenance_timeline(
        ...
        for evt in events_to_record:
            ...
            event = ContractEvent(
                ...
                timestamp=now,
                duration_ms=(
                    total_duration_ms if evt["event_order"] == len(events_to_record) - 1 else None
                ),
            )
            await self.repo.add_event(event)
    ...
    def _to_response_schema(
        ...
        if trust_breakdown_data and isinstance(trust_breakdown_data, dict):
            try:
                tb_dict = {
                    "overall": float(
                        trust_breakdown_data.get("overall", contract.trust_score or 0.90)
                    ),
        ...
        events = [ContractEventSchema.model_validate(e) for e in (contract.events or [])]
        ...
        if contract.merkle_proof:
            ...
            merkle_proof_parsed = [MerkleProofNodeSchema(**mp) for mp in contract.merkle_proof]
