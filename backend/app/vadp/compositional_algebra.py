...
class CompositionalContractAlgebraEngine:
    ...
    def compose(
        ...
        composed_auth = {
            ...
            "result": "allow"
            if auth_trial.get("result") == "allow" and auth_delta.get("result") == "allow"
            else "deny",
        ...
        composed_merkle_root = hashlib.sha256(
            f"0x01:{root_trial}:{hash_delta}".encode()
        ).hexdigest()
        ...
        theorem_3_valid = (
            hashlib.sha256(f"0x01:{root_trial}:{hash_delta}".encode()).hexdigest()
