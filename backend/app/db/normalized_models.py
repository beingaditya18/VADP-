...
class Court(Base, UUIDMixin, TimestampMixin):
    ...
    court_type: Mapped[str] = mapped_column(String(100), default="Supreme Court", index=True, nullable=False)
    jurisdiction: Mapped[str] = mapped_column(String(100), default="Appellate / Original", nullable=False)
...
class Judge(Base, UUIDMixin, TimestampMixin):
    ...
    court_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("courts.id", ondelete="SET NULL"), nullable=True)
...
class Judgment(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    legal_category: Mapped[str] = mapped_column(String(100), index=True, nullable=False)  # Criminal, Civil, etc.
    ...
    review_status: Mapped[str] = mapped_column(String(50), default="under_review", index=True, nullable=False)

    # Relationships
    court: Mapped[Optional[Court]] = relationship("Court", back_populates="judgments")
    parties: Mapped[List[Party]] = relationship("Party", back_populates="judgment", cascade="all, delete-orphan")
    legal_issues: Mapped[List[LegalIssue]] = relationship("LegalIssue", back_populates="judgment", cascade="all, delete-orphan")
    precedents: Mapped[List[Precedent]] = relationship("Precedent", back_populates="judgment", cascade="all, delete-orphan")
    citations: Mapped[List[Citation]] = relationship("Citation", back_populates="judgment", cascade="all, delete-orphan")
    evidence_records: Mapped[List[EvidenceRecordNorm]] = relationship("EvidenceRecordNorm", back_populates="judgment", cascade="all, delete-orphan")
    embeddings: Mapped[List[EmbeddingRecord]] = relationship("EmbeddingRecord", back_populates="judgment", cascade="all, delete-orphan")
    verification_contracts: Mapped[List[VerificationContractNorm]] = relationship("VerificationContractNorm", back_populates="judgment", cascade="all, delete-orphan")
    audit_events: Mapped[List[AuditEventNorm]] = relationship("AuditEventNorm", back_populates="judgment", cascade="all, delete-orphan")
    human_reviews: Mapped[List[HumanReviewNorm]] = relationship("HumanReviewNorm", back_populates="judgment", cascade="all, delete-orphan")
...
class Party(Base, UUIDMixin, TimestampMixin):
    ...
    judgment_id: Mapped[str] = mapped_column(String(36), ForeignKey("judgments.id", ondelete="CASCADE"), index=True, nullable=False)
    party_name: Mapped[str] = mapped_column(String(300), nullable=False)
    party_type: Mapped[str] = mapped_column(String(50), nullable=False)  # petitioner, respondent, appellant, relator
...
class Precedent(Base, UUIDMixin, TimestampMixin):
    ...
    treatment: Mapped[str] = mapped_column(String(100), default="relied_on", nullable=False)  # relied_on, distinguished, overruled
...
class Citation(Base, UUIDMixin, TimestampMixin):
    ...
    judgment_id: Mapped[str] = mapped_column(String(36), ForeignKey("judgments.id", ondelete="CASCADE"), index=True, nullable=False)
    statute_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("statutes.id", ondelete="SET NULL"), nullable=True)
    precedent_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("precedents.id", ondelete="SET NULL"), nullable=True)
    ...
    statute: Mapped[Optional[Statute]] = relationship("Statute", back_populates="citations")
    precedent: Mapped[Optional[Precedent]] = relationship("Precedent", back_populates="citations")
...
class EvidenceRecordNorm(Base, UUIDMixin, TimestampMixin):
    ...
    evidence_type: Mapped[str] = mapped_column(String(100), nullable=False)  # judgment_text, affidavit, forensic, gazette
...
class VerificationContractNorm(Base, UUIDMixin, TimestampMixin):
    ...
    merkle_inclusion_proof: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict, nullable=False)
    
    human_review_status: Mapped[str] = mapped_column(String(50), default="pending_review", nullable=False)
...
class HumanReviewNorm(Base, UUIDMixin, TimestampMixin):
    ...
    judgment_id: Mapped[str] = mapped_column(String(36), ForeignKey("judgments.id", ondelete="CASCADE"), index=True, nullable=False)
    reviewed_by: Mapped[str] = mapped_column(String(255), default="Justice A. K. Sharma", nullable=False)
    review_action: Mapped[str] = mapped_column(String(50), nullable=False)  # approve, reject_override, flag_review
