...
class HSMProvider:
    ...
    def __init__(
        ...
        self.pkcs11_lib_path = pkcs11_lib_path or os.getenv("PKCS11_LIB_PATH", "/usr/lib/softhsm/libsofthsm2.so")
        ...
        self._fallback_key_path: Path = (
            Path(__file__).resolve().parent.parent.parent / "signing_keys" / "ledger_key.pem"
        )
    ...
    def _initialize_hsm(self) -> None:
        ...
        if self.kms_key_id:
            try:
                ...
                logger.info(f"Initialized Cloud KMS Hardware Key Provider with KeyId: {self.kms_key_id}")
                return
            except Exception as e:
                logger.warning(f"Cloud KMS initialization failed ({e}); checking PKCS#11.")
        ...
        try:
            ...
            if os.path.exists(self.pkcs11_lib_path):
                ...
                logger.info(f"Initialized Hardware Security Module via PKCS#11: {self.pkcs11_lib_path}")
                return
        except Exception as e:
            logger.warning(f"PKCS#11 HSM library initialization failed ({e}); engaging software isolated SECP256R1 key fallback.")
        ...
        if key_path.exists():
            try:
                ...
                if isinstance(loaded, ec.EllipticCurvePrivateKey):
                    logger.debug(f"Loaded persistent ECDSA fallback key from {key_path}")
                    return loaded
            except Exception as e:
                logger.warning(f"Failed to load ledger_key.pem ({e}); generating new key.")
    ...
    def sign_digest(self, digest_sha256: bytes) -> bytes:
        ...
        if self.is_kms_active:
            try:
                ...
                logger.error(f"Cloud KMS sign failed ({ex}); attempting HSM/software fallback.")

        if self.is_hsm_active:
            try:
                session = self.pkcs11.openSession(0)
                session.login(self.user_pin)
                key_obj = session.findObjects([(PyKCS11.CKA_LABEL, self.key_label), (PyKCS11.CKA_CLASS, PyKCS11.CKO_PRIVATE_KEY)])[0]
                ...
                logger.error(f"HSM PKCS#11 hardware sign failed ({ex}); falling back to software key.")
    ...
    def get_public_key_pem(self) -> str:
        ...
        if self.is_hsm_active:
            try:
                session = self.pkcs11.openSession(0)
                key_obj = session.findObjects([(PyKCS11.CKA_LABEL, self.key_label), (PyKCS11.CKA_CLASS, PyKCS11.CKO_PUBLIC_KEY)])[0]
