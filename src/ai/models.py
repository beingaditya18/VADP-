...
class AIRecommendation(Base, UUIDMixin, TimestampMixin):
    ...
    explanations: Mapped[list[AIExplanation]] = relationship(
        "AIExplanation", back_populates="recommendation", cascade="all, delete-orphan"
    )
...
class AIExplanation(Base, UUIDMixin, TimestampMixin):
    ...
    explanation_type: Mapped[str] = mapped_column(
        String(100), default="shap_feature_importance", nullable=False
    )
    ...
    natural_language_explanation: Mapped[str | None] = mapped_column(
        Text, nullable=True
    )
    ...
    recommendation: Mapped[AIRecommendation] = relationship(
        "AIRecommendation", back_populates="explanations"
    )
